...
log = getlog(__name__)
p = cf.desktop / 'Activity Record - Jayme Gordon.xlsx'
...
class ExcelModel():
    ...
    def get_df(self, name='ActLog'):
        ...
        if name == 'ActLog':
            ...
            df = df.merge(right=df_sum, how='left', left_index=True, right_index=True)[cols]
    ...
    def display_df(self, n: int = None, df=None, style=None, d_lower=None, **kw):
        ...
        if not style is None:
            df = st.df_terminal_color(df, style)
    ...
    def export_data(self, d_lower: dt = None) -> None:
        """Export task data to generic format

        Parameters
        ----------
        d_lower : dt, optional
            filter by d_lower, default None
        """
        ...
        # rename cols
        cols = ['date', 'time', 'task', 'task_type', 'equipment', 'end_customer']
        m_rename = {c: pu.from_snake(c) for c in cols}

        # filter d_lower
        if not d_lower is None:
            df = df[df.date >= d_lower]
        ...
        m_replace = dict(
            coding='Programming',
            team='Computer Support',
            tsi='TSI Existing Equipment'
        )

        # assign new categories
        df = df \
            .assign(
                task_type=lambda x: x.task_type.str.lower().replace(m_replace, regex=True).str.title(),
                equipment='None',
                end_customer='SMS Service') \
            .rename(columns=dict(duration='time'))[cols] \
            .rename(columns=m_rename)

        # export
        p = cf.desktop / f'Activity Record - Jayme Gordon - {d_lower:%Y-%m-%d}.xlsx'
        df.to_excel(p, index=False)

        log.info(f'Exported {df.shape[0]} rows to {p}')
