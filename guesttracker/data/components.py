...
class ComponentCOConditions(qr.ComponentCOBase):
    def __init__(self, d_lower=None, components=None, minesite='FortHills', **kw):
        ...
        self.cols = [a.UID, a.Unit, a.Title, a.WorkOrder, c.Component,
                     c.Modifier, a.DateAdded, a.SMR, a.ComponentSMR, a.Floc]
...
def get_reman_db_path():
    ...
    p = Path(cf.config['FilePaths']['RemanDB'])
...
def load_stupid_fucking_reman_db(p=None):
    ...
    cols = [
        'rep_date', 'rec_date', 'branch', 'customer', 'unit_num', 'model', 'machine_ser',
        'component', 'comp_loc', 'comp_serial', 'machine_hrs', 'comp_hrs', 'report_type',
        'smswo', 'branch_wo', 'origination', 'work_type', 'comp_issue']
    ...
    df = fl.read_access_database(p=p, table_name='CondRepTbl', index_col='ID')[cols] \
...
def load_basemine_componennt_db(p=None):
    if p is None:
        p = cf.config['FilePaths']['BaseMineCompDB']
...
def convert_basemine_component(df):
    ...
    # merge floc
    df_comp = db.get_df_component() \
        .pipe(lambda df: df[df.EquipClass == 'Truck_Electric'])[['Component', 'Modifier', 'Floc', 'Combined']]

    return df \
        .replace(m_conv)[m_cols] \
...
def df_ac_motor_wos():
    p = cf.desktop / 'ac_motor_bands.xlsx'
    return pd.read_excel(p, sheet_name=1) \
        .pipe(f.lower_cols)


def df_srb_wos():
    p = cf.desktop / 'srb_wos.csv'
...
def read_comp_co_accounting():
    ...
    p = cf.desktop / 'accounting_comp_co.xlsx'
...
def read_suncor_comp_sales():
    p = cf.desktop / 'suncor_comp_sales.xlsx'
...
def merge_sun_comp_reman(df_sms, df_reman, df_acc):
    ...
    >>> p_reman = cf.desktop / '2021-03-01 - Master Product Reports_be.accdb'
