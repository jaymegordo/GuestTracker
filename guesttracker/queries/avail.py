...
class AvailBase(QueryBase):
    ...
    def set_default_args(self, d_rng: Tuple[dt, dt]):
        self.add_fltr_args([
...
class AvailSummary(QueryBase):
    ...
    def from_name(cls, name, period='month', **kw):
        ...
        row = df_period(freq=period, n_years=5).loc[name]
    ...
    def q(self):
        ...
        hrs_in_period = cfn('tblHrsInPeriod', ['d_lower', 'd_upper', 'minesite', 'period'])
    ...
    def process_df(self, df):
        ...
        p = cf.p_res / 'csv/ma_guarantee.csv'
        ...
        # Series.view(int) doesn't work on windows (doubles length of array somehow, problem with numpy)
        # _age=lambda x: x.d_upper.dt.to_period('M').view(int) - x.DeliveryDate.dt.to_period('M').view(int),

        return df \
            .fillna(dict(ExcludeHours_MA=0, ExcludeHours_PA=0)) \
            .assign(
                ...
                _age=lambda x: (x.d_upper.dt.to_period('M') - x.DeliveryDate.dt.to_period('M')).apply(lambda i: i.n),
                ...
                hrs_period=lambda x: ((
                    ...
                    np.maximum(
                        x.DeliveryDate,
                        x.period.dt.start_time)).dt.days + 1) * 24,
    ...
    def df_report(self, period='last'):
        ...
        cols = ['Model', 'Unit', 'Total', 'SMS', 'Suncor', 'ma_target',
                'MA', 'hrs_period_ma', 'PA', 'hrs_period_pa', 'Operation']
    ...
    def style_totals(self, style):
        m_fmt = self.formats | {k: '{:,.0f}' for k in ('Total', 'SMS', 'Suncor')}
        return style \
    ...
    def df_history_rolling(self, prd_str=False, totals=False, merge_f300=False, **kw) -> pd.DataFrame:
        ...
        return self.df_summary(group_ahs=False) \
            .assign(period=lambda x: x.period.dt.strftime(self.fmt_str) if prd_str else x.period)[cols] \
    ...
    def merge_f300(self, df, query_f300=None, do=False, **kw):
        ...
        df_smr = query_f300.df_monthly()[['SMR Operated']]
    ...
    def update_style(self, style, outlook=False, **kw):
        ...
        # HACK need smaller font size for report. add_table_attrs doesn't work for multiple 'style' values,
        # need to overwrite completely.
...
class AvailRawData(AvailBase):
    def __init__(self, da=None, **kw):
        ...
        a = self.a
        cols = [a.Unit, a.ShiftDate, a.StartDate, a.EndDate, a.Duration.as_(
            'Total'), a.SMS, a.Suncor, a.CategoryAssigned.as_('Category Assigned'), a.DownReason, a.Comment]
    ...
    def background_gradient(self, style: 'Styler', theme: str = None, do: bool = True) -> 'Styler':
        if not do:
            return style

        if theme is None:
            theme = self.theme  # Usually dark, but could be light for email
    ...
    def highlight_category_assigned(self, style: 'Styler') -> 'Styler':
        if self.theme == 'dark':
            return style
        ...
        return style.apply(
            func=st.highlight_multiple_vals,
    ...
    def update_style(self, style: 'Styler', **kw) -> 'Styler':
        ...
        col_widths = dict(StartDate=65, EndDate=65, Total=20, SMS=20, Suncor=20, Comment=150) \
            | {'Category Assigned': 80}
        ...
        return style \
            ...
            .hide(subset=['DownReason'], axis='columns')
...
class Availability(AvailRawData):
    ...
    def process_df(self, df: pd.DataFrame) -> pd.DataFrame:
        if df.shape[0] == 0:
            log.warning('No rows returned from avail table.')
            return df
        ...
        ca = 'Category Assigned'
        new_rows = df[ca].isnull()

        df.loc[new_rows, ca] = df \
            .merge(
                right=df_assigned,
                how='left',
                on='DownReason')[f'{ca}_y']

        # rules to change assignement based on comments
        m = {
            r'air.*filter': 'Air Filters',
            'hub|spindle': 'Spindle',
            'batter': 'Batteries',
            'light': 'Lights'}
        for expr, repl in m.items():
            df.loc[new_rows, ca] = where(
                df.Comment[new_rows].str.contains(expr, case=False),
                repl,
                df[ca][new_rows])
        ...
        # comments without number (eg 2000hr) will be 0 = s5 service
        expr = r'(\d{3,})'
        serv = df \
            .loc[(
                new_rows &
                df.DownReason.str.lower().str.contains('service') &
                df.Comment.notnull())] \
            .Comment.str.extract(expr).fillna(0).astype(int) % 2000

        # assign s5, s4, or s1 service to new rows loc
        df.loc[serv.index, ca] = \
            where(serv == 0, 'S5 Service',
                  where(serv == 1000, 'S4 Service', 'S1 Service'))

        # match CategoryAssigned to SMS or suncor and fill duration
        return df.merge(right=df_resp, on=ca, how='left') \
            .assign(
                SMS=lambda x: where(x.SMS.isnull(), where(x.Resp == 'SMS', x.Total, 0), x.SMS),
                Suncor=lambda x: where(x.Suncor.isnull(), where(x.Resp == 'Suncor', x.Total, 0), x.Suncor)) \
            .drop(columns='Resp')
