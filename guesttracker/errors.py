...
def global_exception_hook_local(*exc_info) -> None:
    """Custom exception hook to catch all errors in central location
    - local testing only, don't send to sentry
...
def wrap_all_class_funcs(cls, err_func=None):
    ...
    if err_func is None:
        err_func = getattr(sys.modules[__name__], 'e')
...
def decorate_all_classes(module_name: str = None, module=None, err_func=None):
...
def display_error(
    ...
    if isinstance(exc, SMSDatabaseError) or (not log is None and 'database' in log.name):
        msg = f'{msg}\nIf this is a database or network related error, ' \
            + 'check your network connection, then try doing Database > Reset Database Connection.'
...
class AppNotOpenError(ExpectedError):
    """Raised if desired app not running."""

    def __init__(self, app: str = 'Outlook'):
        message = f'App "{app}" not open.',
        super().__init__(message)

        msg = f'WARNING: "{app}" not open. Please start the app and retry.'
        self.update_statusbar(msg=self.msg)
...
class SettingsError(ExpectedError):
    """Raised no saved settings exist."""

    def __init__(self, message='No saved data.'):
        super().__init__(message, warn=True)


class ReadOnlyError(ExpectedError):
    """Raised if table or global app in read_only mode."""

    def __init__(self, message='Application or table in READ_ONLY mode, cannot edit cell value.'):
        super().__init__(message, warn=True)
...
class NoUnitError(ExpectedError):
    """Raised if unit number cannot be identified"""

    def __init__(
            self,
            message='ERROR: No unit number identified from files. Add unit number to folder name.'):
        super().__init__(message)

        self.update_statusbar(msg=message)


class NoDateError(ExpectedError):
    """Raised if date cant be identified from files"""

    def __init__(
            self,
            message='ERROR: No date identified from files. Add date to folder name in format YYYY-DD-MM.'):
        super().__init__(message)

        self.update_statusbar(msg=message)


class FolderExistsError(ExpectedError):
    """Raised if file/folder exists and dont want to overwrite"""

    def __init__(self, p):
        msg = f'WARNING: Folder exists, can\'t overwrite: {p}'
        super().__init__(msg)
...
if IS_QT_APP or AZURE_WEB:
    ...
    sys.excepthook = global_exception_hook_local
